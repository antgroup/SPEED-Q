...
class STEFunction(torch.autograd.Function):
    """用于实现 Straight-Through Estimator 的自定义函数"""
    @staticmethod
    def forward(ctx, x, scale, zero, maxq, eps=1e-9):
        # 保存 scale 和 zero 用于反向传播
    ...
    def backward(ctx, grad_output):
        ...
        # 计算梯度掩码,将超出量化范围的位置的梯度置为0
        mask = (x >= -scale * zero) & (x <= scale * (ctx.maxq - zero))
        grad_input = grad_output * mask.float()
        
        # 对 scale 和 zero 的梯度设为 None,因为它们在训练中不需要更新
        return grad_input, None, None, None, None


def quantize_dequantize_ste(x, scale, zero, maxq, eps=1e-9):
    return STEFunction.apply(x, scale, zero, maxq, eps)
...
class BilevelQuantizer(nn.Module):
    def __init__(self, shape=1):
        super(BilevelQuantizer, self).__init__()
    ...
    def configure(
        ...
        self.qq_scale_sym = qq_scale_sym
    ...
    def find_params(self, x, weight=False):
        ...
        maybe_round_zero = (lambda x: torch.round(x).clamp(0, torch.tensor(2**self.qq_zero_bits-1))) if self.round_zero else lambda x: x
        ...
        self.scale = ((xmax - xmin) / self.maxq).clamp_min(1e-5)        
        if self.sym:
        ...
        if self.qq_scale_bits is not None:
            ...
            self.qq_scale = BilevelQuantizer(shape=scale_groups.shape)
            self.qq_scale.configure(self.qq_scale_bits, perchannel=True, sym=self.qq_scale_sym, round_zero=False, **self.qqq_params)
        ...
        if self.qq_zero_bits is not None and ((not self.round_zero) or self.qq_zero_bits < self.bits):
            ...
            self.qq_zero = BilevelQuantizer(shape=zero_groups.shape)
    ...
    def quantize_dequantize(self, x):
        if self.ready():
            ...
            return quantize_dequantize_ste(x, self.scale, self.zero, self.maxq)
        return x
